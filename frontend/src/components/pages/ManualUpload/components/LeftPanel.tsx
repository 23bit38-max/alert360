...
import { Upload, X, RefreshCw, AlertTriangle, CheckCircle, Activity, Cpu, Wifi, Maximize2, Zap, Target, Eye } from 'lucide-react';
import { motion, AnimatePresence } from 'framer-motion';
import type { UploadedFile, DetectionResult } from '../types';
...
export const LeftPanel: React.FC<LeftPanelProps> = ({
    ...
    useEffect(() => {
        if (detectionResult) {
            const interval = setInterval(() => {
                setFps(prev => Math.max(24, Math.min(60, prev + (Math.random() * 4 - 2))));
                setLatency(prev => Math.max(20, Math.min(80, prev + (Math.random() * 10 - 5))));
            }, 1000);
    ...
    const renderBoundingBoxes = () => {
        if (!detectionResult?.boxes || !mediaDimensions.naturalWidth) return null;

        const scaleX = mediaDimensions.width / mediaDimensions.naturalWidth;
        const scaleY = mediaDimensions.height / mediaDimensions.naturalHeight;

        // Offset calculation for object-contain (simpilfied for aspect-video container)
        // Since we are in aspect-video which usually matches 16:9, and most feeds are 16:9,
        // we assume minimal letterboxing for now or that it fills the box.

        return (
            <svg
                className="absolute inset-0 pointer-events-none z-20 w-full h-full"
                viewBox={`0 0 ${mediaDimensions.width} ${mediaDimensions.height}`}
            >
                {detectionResult.boxes.map((box, i) => {
                    ...
                    return (
                        <g key={i} className="animate-box-in">
                            {/* Animated Corner Brackets for Bounding Box */}
                            <path
                                d={`M ${left} ${top + 10} L ${left} ${top} L ${left + 10} ${top}`}
                                fill="none"
                                stroke={detectionResult.accidentDetected ? "#ef4444" : "#22c55e"}
                                strokeWidth="1.5"
                                className="box-corner"
                            />
                            <path
                                d={`M ${left + width - 10} ${top} L ${left + width} ${top} L ${left + width} ${top + 10}`}
                                fill="none"
                                stroke={detectionResult.accidentDetected ? "#ef4444" : "#22c55e"}
                                strokeWidth="1.5"
                                className="box-corner"
                            />
                            <path
                                d={`M ${left + width} ${top + height - 10} L ${left + width} ${top + height} L ${left + width - 10} ${top + height}`}
                                fill="none"
                                stroke={detectionResult.accidentDetected ? "#ef4444" : "#22c55e"}
                                strokeWidth="1.5"
                                className="box-corner"
                            />
                            <path
                                d={`M ${left + 10} ${top + height} L ${left} ${top + height} L ${left} ${top + height - 10}`}
                                fill="none"
                                stroke={detectionResult.accidentDetected ? "#ef4444" : "#22c55e"}
                                strokeWidth="1.5"
                                className="box-corner"
                            ...
                            <rect
                                ...
                                height={height}
                                fill="none"
                                stroke={detectionResult.accidentDetected ? "#ef444433" : "#22c55e33"}
                                strokeWidth="1"
                                strokeDasharray="4 4"
                                className="animate-pulse-subtle"
                            />

                            {/* Ghost Identifier (Ultra-minimal, Tiny Text) */}
                            <g transform={`translate(${left}, ${top - 3})`}>
                                <text
                                    x={0}
                                    y={0}
                                    fill={detectionResult.accidentDetected ? "#ef4444" : "#22c55e"}
                                    fontSize="5.5"
                                    fontWeight="900"
                                ...
                                >
                                    {(() => {
                                        const raw = detectionResult.label?.split(':')[0].toUpperCase() || 'SCAN';
            ...
            <div className={`flex-1 relative flex flex-col ${!hasFiles ? 'justify-center min-h-[400px]' : 'min-h-0'}`}>
                        ...
                        <div className="text-center p-12 animate-in fade-in zoom-in duration-700">
                            ...
                            <div className="space-y-3">
                                ...
                                <p className="text-[9px] font-bold text-muted-foreground/40 uppercase tracking-widest">
                                    Auth Protocol: YOLOv11 Engine • Secure Channel
                                </p>
                            </div>
                            ...
                            <AnimatePresence>
                                {uploading && (
                                    <motion.div
                                        ...
                                        animate={{ opacity: 1 }}
                                        exit={{ opacity: 0 }}
                                        className="absolute inset-0 bg-black/90 backdrop-blur-md flex flex-col items-center justify-center z-30"
                                    >
                                        <div className="relative w-24 h-24 mb-6">
                                            <motion.div
                                                animate={{ rotate: 360 }}
                                                transition={{ duration: 2, repeat: Infinity, ease: "linear" }}
                                                className="absolute inset-0 border-[3px] border-primary border-t-transparent rounded-full shadow-[0_0_20px_rgba(16,185,129,0.3)]"
                                            />
                                            <div className="absolute inset-0 border-[3px] border-white/5 rounded-full" />
                                            <div className="absolute inset-0 flex items-center justify-center">
                                                <Eye className="w-8 h-8 text-primary/50 animate-pulse" />
                                            </div>
                                        </div>
                                        <motion.p
                                            animate={{ opacity: [0.4, 1, 0.4] }}
                                            transition={{ duration: 2, repeat: Infinity }}
                                            className="text-primary text-[10px] font-black tracking-[0.4em]"
                                        >
                                            BOOTING AI CORE
                                        </motion.p>
                                        <p className="text-[9px] font-bold text-muted-foreground mt-3 uppercase tracking-widest opacity-50">{uploadProgress}% SYNCED</p>
                                    </motion.div>
                                )}
                            </AnimatePresence>
                        </div>
                ...
                {hasFiles && (
                    <div className="grid grid-cols-4 gap-px bg-white/5 border-b border-white/5">
                        {[
                            { icon: Activity, label: 'FPS', value: fps.toFixed(0), color: 'text-primary' },
                            { icon: Wifi, label: 'LATENCY', value: `${latency}ms`, color: 'text-blue-400' },
                            { icon: Cpu, label: 'ENGINE', value: 'YOLOv11', color: 'text-purple-400' },
                            { icon: Maximize2, label: 'RES', value: '1080p', color: 'text-amber-500' }
                        ].map((stat, i) => (
                            <div key={i} className="bg-white/[0.01] p-4 flex flex-col items-center justify-center gap-1.5 hover:bg-white/[0.03] transition-colors">
                                ...
                                <span className="text-[10px] font-black text-white tracking-widest font-mono">{stat.value}</span>
                            </div>
                        ))}
                    </div>
                )}
